...
public class PrintUtils {
	...
	/**
	 * Replaces in the given string, all occurrences of the tab control sequence
	 * (<code>\t</code>) with the given amount of spaces.<br />
	 ...
	 * 
	 * @param str
	 *            The source string to replace the tabs in
	 * @param nbSpaces
	 *            The amount of spaces with which the tab should get replaced
	...
	/**
	 * Replaces in the givne string all occurrences of <code>\t</code> with four
	 * spaces. <br />
	 * A call of this method returns the exact same value as invoking
	 * {@link #replaceTabBySpaces(String, int)} with <tt>str</tt> as string
	 * parameter and <tt>4</tt> as integer parameter.
	 * 
	 * @param str
	 *            The string whichs tabs get replaced
	...
	private static String wrapLinesInternal(String[] lines, int length) {
		...
		for (String l : lines) {
			...
			if (s.length() <= length) {
				...
				int index = s.lastIndexOf(" ", length); // herzlichen Dank an
														// Silvan Stich!
				if (index > s.length()) {
